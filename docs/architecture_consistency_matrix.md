...
| Weaviate semantic object-property memory | `docs/architecture.md` | `MelampoWeaviateSchema`, `WeaviateEnterpriseMemoryAdapter` | local-fallback | Provenance, learning status and license metadata required. **Superseded**: graph and vector storage decided as FalkorDB instead, see the row below and `docs/recursive_engine_decision_record.md` -- Weaviate was never connected to a real service either, and this component is not being built out further |
| Concept graph traversal and storage | `docs/recursive_engine_decision_record.md` | `memory/concept_paths.py` (`ConceptGraphView` protocol), `memory/falkordb_graph.py` (`FalkorConceptGraph`) | implemented | Two backends behind one protocol, verified identical output: `InMemoryConceptGraph` (in-process, the reference implementation) and `FalkorConceptGraph` (FalkorDB, embedded or remote via one config file, `data/falkordb_config.toml`). Chosen over Weaviate and Qdrant because the project's core operation is graph traversal, not similarity search -- FalkorDB is the only one of the three considered that does both natively |
