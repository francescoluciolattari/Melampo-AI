...
| `pms_ema.py` | EMA Product Management Service | Farmaci autorizzati centralmente nell'UE, FHIR R5, **beta pubblica confermata attiva** (verificato contro il FAQ ufficiale EMA e la UPD Registration Guide) | **Richiesta** -- OAuth2 client-credentials su tenant Microsoft Entra ID di EMA (`PMS_EMA_CLIENT_ID` / `PMS_EMA_CLIENT_SECRET`, non più una chiave statica) | **500 richieste/minuto per IP** -- confermato dal FAQ ufficiale EMA (429 + `Retry-After` oltre il limite); mantenuto ben al di sotto |
